...
表达式（类型为 `Expr` 的项）是 Lean 程序的抽象语法树。这意味着每个可以用 Lean 编写的项都有一个对应的 `Expr`。例如，应用 `f e` 由表达式 `Expr.app ⟦f⟧ ⟦e⟧` 表示，其中 `⟦f⟧` 是 `f` 的表示，而 `⟦e⟧` 是 `e` 的表示。类似地，项 `Nat` 由表达式 ``Expr.const `Nat []`` 表示。（反引号和空列表将在下面讨论。）
...
- `lit` 是一个 **字面值**，这是一个数字或字符串文字，如 `4` 或 `"hello world"`。字面值有助于提高性能：我们不想将表达式 `(10000 : Nat)` 表示为 `Nat.succ $ ... $ Nat.succ Nat.zero`。
